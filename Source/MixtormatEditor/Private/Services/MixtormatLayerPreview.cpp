#include "Services/MixtormatLayerPreview.h"
...
namespace MixtormatLayerPreview
{
...
UMaterial* FMixtormatLayerPreview::CreateMaterial()
{
	using namespace MixtormatLayerPreview;
...
void FMixtormatLayerPreview::ApplyLayers(
	UMaterialInstanceDynamic& MaterialInstance,
	const TArray<FMixtormatLayer>& Layers)
{
	...
	for (int32 LayerIndex = 0; LayerIndex < LayerCount; ++LayerIndex)
	{
		const FMixtormatLayer* Layer = Layers.IsValidIndex(LayerIndex)
			? &Layers[LayerIndex]
			: nullptr;
		const bool bEnabled = Layer && Layer->bEnabled;
		const UMixtormatSurface* Surface = Layer
		...
		SetScalar(MaterialInstance, LayerIndex, TEXT("Coat"), Layer && Layer->CompositionMode == EMixtormatCompositionMode::Coat ? 1.0f : 0.0f);
		SetScalar(
			MaterialInstance,
			LayerIndex,
			TEXT("Tiling"),
