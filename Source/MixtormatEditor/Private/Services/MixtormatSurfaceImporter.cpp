...
namespace MixtormatImporter
{
		...
		FMixtormatImportResult& Result)
	{
		...
		if (!FMixtormatNormalHeightGenerator::Generate(
			NormalPixels,
			NormalSize,
	...
	UMixtormatSurface* CreateOrLoadSurface(
		IAssetTools& AssetTools,
		const FString& AssetName,
		const FString& DestinationPath)
	{
		const FString ObjectPath = FString::Printf(
			TEXT("%s/%s.%s"), *DestinationPath, *AssetName, *AssetName);
		if (UMixtormatSurface* ExistingSurface = LoadObject<UMixtormatSurface>(nullptr, *ObjectPath))
		{
			return ExistingSurface;
		}

		UDataAssetFactory* Factory = NewObject<UDataAssetFactory>();
		Factory->DataAssetClass = UMixtormatSurface::StaticClass();
		return Cast<UMixtormatSurface>(AssetTools.CreateAsset(
			AssetName,
			DestinationPath,
...
FText FMixtormatImportResult::ToMessage() const
{
	if (bCancelled)
	{
		return NSLOCTEXT("MixtormatImporter", "Cancelled", "Import cancelled.");
	}
...
FMixtormatImportResult FMixtormatSurfaceImporter::ImportShippedMasks()
{
...
FMixtormatImportResult FMixtormatSurfaceImporter::ImportShippedNormals()
{
	using namespace MixtormatImporter;
	FMixtormatImportResult Result;
	const FString TexturesRoot = GetPluginTexturesRoot();
...
FMixtormatImportResult FMixtormatSurfaceImporter::ImportShippedEffects()
{
	using namespace MixtormatImporter;
	FMixtormatImportResult Result;
	const FString TexturesRoot = GetPluginTexturesRoot();
...
FString FMixtormatSurfaceImporter::GetDefaultSourceDirectory()
{
	const TArray<FString> Directories = EnumerateShippedSourceDirectories();
	return Directories.IsEmpty() ? FString() : Directories[0];
}

FMixtormatImportResult FMixtormatSurfaceImporter::ImportDefaultLibrary()
{
	FMixtormatImportResult Result;
	...
	const FMixtormatImportResult EffectStep = ImportShippedEffects();
...
FMixtormatImportResult FMixtormatSurfaceImporter::ReimportShippedLibrary()
{
...
FMixtormatImportResult FMixtormatSurfaceImporter::ImportDirectory(const FString& SourceDirectory)
{
	...
	for (const TPair<FString, FTextureSet>& Pair : TextureSets)
	{
		...
		EMixtormatBlendHeightProvenance HeightProvenance = bHasAuthoredHeight
			? EMixtormatBlendHeightProvenance::AuthoredRAMH
		...
		if (!bHasAuthoredHeight)
		{
			...
			{
				Ram = DerivedRAMH;
				HeightProvenance = EMixtormatBlendHeightProvenance::DerivedFromNormal;
			}
		}
