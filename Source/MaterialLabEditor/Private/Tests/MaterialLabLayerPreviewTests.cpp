...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMaterialLabGpuCompositorTest,
	"MaterialLab.Editor.GpuCompositor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FMaterialLabGpuCompositorTest::RunTest(const FString& Parameters)
{
	FMaterialLabGpuCompositor Compositor;
	...
	TestTrue(
		...
		LegacyLayer.Children[0].Type == EMaterialLabLayerChildType::Mask
			&& LegacyLayer.Children[1].Type == EMaterialLabLayerChildType::Effect);
	TestTrue(
		TEXT("Legacy arrays are cleared after migration"),
		LegacyLayer.Masks.IsEmpty() && LegacyLayer.Effects.IsEmpty());

	FMaterialLabLayer BaseLayer;
	BaseLayer.Type = EMaterialLabLayerType::Fill;
	...
	TestTrue(
		...
		BaseLayer.HeightSource == EMaterialLabHeightSource::LayerHeight);
	...
	FMaterialLabLayer TopLayer = BaseLayer;
	...
	UTexture2D* PeelTexture = LoadObject<UTexture2D>(
		nullptr,
		TEXT("/Engine/EngineResources/WhiteSquareTexture.WhiteSquareTexture"));
	UMaterialLabEffect* PeelingAsset = NewObject<UMaterialLabEffect>(GetTransientPackage());
	...
	if (PeelTexture && PeelingAsset)
	{
		...
		FMaterialLabLayerChild& PeelingChild = Layers[1].Children.AddDefaulted_GetRef();
		PeelingChild.Type = EMaterialLabLayerChildType::Effect;
		FMaterialLabLayerEffect& Peeling = PeelingChild.Effect;
	...
	Layers[1].Metallic = 0.0f;
	Layers[1].CompositionMode = EMaterialLabCompositionMode::Coat;
	...
	Layers[1].CompositionMode = EMaterialLabCompositionMode::Replace;
	Layers[1].bOverrideIOR = true;
	Layers[1].IOR = 2.0f;
	...
	if (WhiteMask)
	{
		...
		{
			FMaterialLabLayerChild& EffectFirst = Layers[1].Children.AddDefaulted_GetRef();
			EffectFirst.Type = EMaterialLabLayerChildType::Effect;
			EffectFirst.Effect.Effect = PeelingAsset;
			EffectFirst.Effect.Front = 2.0f;
			FMaterialLabLayerChild& HalfMask = Layers[1].Children.AddDefaulted_GetRef();
			HalfMask.Type = EMaterialLabLayerChildType::Mask;
	...
	Layers[1].HeightSource = EMaterialLabHeightSource::Constant;
	...
	Layers[1].bHeightBlendEnabled = true;
	Layers[1].HeightSource = EMaterialLabHeightSource::LayerHeight;
	Layers[1].ConstantHeight = 0.4f;
	if (PeelTexture)
	{
		FMaterialLabLayerChild& HeightMask = Layers[1].Children.AddDefaulted_GetRef();
		HeightMask.Type = EMaterialLabLayerChildType::Mask;
